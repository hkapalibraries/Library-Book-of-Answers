...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!grokApiKey) {
      console.warn('GROK_API_KEY missing in Vercel Function.');
      return res.status(502).json({ error: 'Missing API Key in Vercel environment' });
    }
...
`;
    ...
    if (!response.ok) {
      ...
      console.error('Grok API error in Vercel Function:', errorData);
    ...
    console.error('Divination error in Vercel Function:', error);
